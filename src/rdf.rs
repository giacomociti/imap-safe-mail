...
impl Default for RdfOptions {
    fn default() -> Self {
        Self {
            data_iri: "https://example.org/data/".into(),
            graph: None,
            include_body: false,
        }
    }
...
pub fn message_to_nquads(message: &FetchedMessage, options: &RdfOptions) -> String {
    ...
    let message_iri = match message_id {
        ...
        _ => format!(
            "{base}msg/imap/{}/{}",
            percent_encode(&message.source.account),
            message.uid
        ),
    };
    ...
    if let Some(value) = subject {
        ...
        literal(
            &mut lines,
            &message_iri,
            &format!("{MAIL}normalizedSubject"),
            &normalize_subject(value),
            None,
            options,
        );
    }
    for (header, predicate) in [
        ("from", "from"),
        ("to", "to"),
        ("cc", "cc"),
        ("bcc", "bcc"),
    ...
    if let Some(agent) = headers
        .get("user-agent")
        .or_else(|| headers.get("x-mailer"))
    {
        literal(
            &mut lines,
            &message_iri,
            &format!("{MAIL}userAgent"),
            agent,
            None,
            options,
        );
...
/// A SPARQL 1.1 Update which removes the old projection for an IMAP UID.
/// The match uses `mail:sourcePath`, so it also works for messages whose
/// resource IRI was derived from a Message-ID.
pub fn delete_message_update(message: &FetchedMessage, graph: &str) -> String {
    format!(
        "DELETE {{ GRAPH <{graph}> {{ ?message ?predicate ?object }} }}\nWHERE {{ GRAPH <{graph}> {{ ?message <{MAIL}sourcePath> \"{}\" ; ?predicate ?object }} }};\n",
        escape(&message.source_uri())
    )
}

/// A SPARQL 1.1 Update which removes the old projection for a vanished UID.
pub fn delete_uid_update(account: &str, mailbox: &str, uid: u32, graph: &str) -> String {
    let source = format!("imap://{account}/{mailbox};uid={uid}");
    format!(
        "DELETE {{ GRAPH <{graph}> {{ ?message ?predicate ?object }} }}\nWHERE {{ GRAPH <{graph}> {{ ?message <{MAIL}sourcePath> \"{}\" ; ?predicate ?object }} }};\n",
        escape(&source)
    )
...
/// Delete all message projections for exactly one IMAP account/mailbox. Used
/// only to replace a complete snapshot (initial sync or UIDVALIDITY reset).
pub fn delete_mailbox_update(account: &str, mailbox: &str, graph: &str) -> String {
    let prefix = format!("imap://{account}/{mailbox};uid=");
    format!(
        "DELETE {{ GRAPH <{graph}> {{ ?message ?predicate ?object }} }}\nWHERE {{ GRAPH <{graph}> {{ ?message <{MAIL}sourcePath> ?source ; ?predicate ?object . FILTER(STRSTARTS(STR(?source), \"{}\")) }} }};\n",
        escape(&prefix)
    )
}

/// Insert one message projection into a named graph. `message_to_nquads` is
/// deliberately called with no graph, yielding the N-Triples required by
/// SPARQL `INSERT DATA`.
pub fn insert_message_update(
    message: &FetchedMessage,
    graph: &str,
    options: &RdfOptions,
) -> String {
    let triples = message_to_nquads(
        message,
        &RdfOptions {
            graph: None,
            ..options.clone()
...
mod tests {
    ...
    fn projection_uses_the_mbox_rdf_namespaces_and_named_graphs() {
        ...
        let rdf = message_to_nquads(
            &message,
            &RdfOptions {
                graph: Some("urn:email:alice@example.org".into()),
                include_body: true,
                ..Default::default()
            },
        );
